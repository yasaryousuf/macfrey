    ...
    <div class="col-md-12 grid-margin stretch-card">
        <div class="card">
            <div class="card-body">
                ...
                <form action="{{route('news.update', ['id' => $news->id])}}" method="POST" name="create-news" enctype="multipart/form-data">
                    @method('PUT')
                    @csrf
                {{-- <input type="hidden" name="id" value="{{$news->id}}"> --}}
                    <div class="form-group">
                        <label for="exampleInputName1">Title</label>
                    <input type="text" class="form-control" placeholder="Name" name="title" value="{{$news->title}}" required>
                    </div>
                    <img id="news-image-preview" alt="your image"  height="200" style="margin-bottom:20px" src="{{$news->thumbnail ? asset('/images/news/thumbnails/'. $news->thumbnail ) : asset('/images/No_Image.svg')}}"/>
